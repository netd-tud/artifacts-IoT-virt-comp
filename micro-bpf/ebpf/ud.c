...
/* ==========================================
   ...
   Details:
      ...
      - provided loop based memcmp
      - added int main
      - inlined functions
      - added signed division method.
   ========================================== */
...
static inline int sdiv(uint32_t dividend, uint32_t divisor) {
    // eBPF support division by 0 (the result is also 0)
    uint8_t negative = ((dividend < 0) ^ (divisor < 0)) ? 1 : 0;
...
int32_t verify_benchmark(int res)
{
