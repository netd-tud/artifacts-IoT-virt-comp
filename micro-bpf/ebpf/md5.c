...
/* ==========================================
   ...
   Details:
      ...
      - provided loop based memcpy and memset implementation
      - added main
      - inlined functions
      - removed debug prints
      - moved form the beeps allocator to RIOT's memarray
   ========================================== */
...
#define MSG_SIZE_PADDED (((((MSG_SIZE + 8) / 64) + 1) * 64) - 8)
...
static uint8_t starting_message[MSG_SIZE];
static uint8_t msg_buff[MSG_SIZE_PADDED + 64];
...
void md5(uint8_t *initial_msg)
{
    ...
    msg = msg_buff;
    memset(msg, 0, sizeof(msg_buff));

    memcpy(msg, initial_msg, MSG_SIZE);
    msg[MSG_SIZE] = 128; // write the "1" bit

    uint32_t bits_len = 8 * MSG_SIZE;            // note, we append the len
    memcpy(msg + MSG_SIZE_PADDED, &bits_len, 4); // in bits at the end of the buffer

    // Process the message in successive 512-bit chunks:
    //for each 512-bit chunk of message:
    int offset;
    for (offset = 0; offset < MSG_SIZE_PADDED; offset += (512 / 8)) {
