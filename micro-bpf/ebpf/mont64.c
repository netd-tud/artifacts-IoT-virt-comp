...
/* ==========================================
   ...
   Details:
      ...
      - added main
      - inlined functions
      - removed the mulul64 funciton that utilizes the __int128 builtin type
   ========================================== */
...
/* Multiply uint32_t 64-bit routine, i.e., 64 * 64 ==> 128.
...
void xbinGCD(uint64_t a, uint64_t b, volatile uint64_t *pu, volatile uint64_t *pv)
{
...
/* ------------------------------ main ------------------------------ */
static uint64_t in_a, in_b, in_m;
...
static inline int benchmark_body(unsigned int lsf)
{
    ...
    for (uint32_t lsf_cnt = 0; lsf_cnt < lsf; lsf_cnt++) {
        uint64_t a, b, m, hr, p1hi, p1lo, p1, p, abar, bbar;
        uint64_t phi, plo;
        volatile uint64_t rinv, mprime;
        errors = 0;
        ...
        /* The simple calculation: This computes (a*b)**4 (mod m) correctly for all a, b,
         * m < 2**64. */
