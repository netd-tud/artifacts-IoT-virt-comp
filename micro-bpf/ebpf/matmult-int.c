...
/* ==========================================
   ...
   Details:
      ...
      - provided loop based memcpy and memcmp
      - added main
      - inlined functions
      - removed warm_caches, InitSeed
   ========================================== */
...
/* This scale factor will be changed to equalise the runtime of the
   benchmarks. */
#ifndef SCALE_FACTOR
#  define SCALE_FACTOR 39
#endif /* ifndef SCALE_FACTOR */

#define UPPERLIMIT         20
#define ZERO               0
#define MOD_SIZE           8095
typedef int32_t matrix[UPPERLIMIT][UPPERLIMIT];

static inline int8_t values_match(int32_t v1, int32_t v2)
{
    return (v1 == v2);
}
...
int32_t seed;
matrix array_a_ref, array_a, array_b_ref, array_b, result_array;
...
static inline int benchmark_body(unsigned int lsf)
{
    ...
    for (uint32_t lsf_cnt = 0; lsf_cnt < lsf; lsf_cnt++) {
        memcpy(array_a, array_a_ref,
               UPPERLIMIT * UPPERLIMIT * sizeof(array_a[0][0]));
        memcpy(array_b, array_b_ref,
               UPPERLIMIT * UPPERLIMIT * sizeof(array_a[0][0]));

        test(array_a, array_b, result_array);
    }
...
void multiply(matrix A, matrix B, matrix Res)
{
    register int32_t Outer, Inner, Index;
...
void initialise_benchmark()
{
    ...
    for (OuterIndex = 0; OuterIndex < UPPERLIMIT; OuterIndex++)
        for (InnerIndex = 0; InnerIndex < UPPERLIMIT; InnerIndex++)
            array_a_ref[OuterIndex][InnerIndex] = (int32_t) random_integer();
    for (OuterIndex = 0; OuterIndex < UPPERLIMIT; OuterIndex++)
        for (InnerIndex = 0; InnerIndex < UPPERLIMIT; InnerIndex++)
            array_b_ref[OuterIndex][InnerIndex] = (int32_t) random_integer();
}
...
int verify_benchmark(int unused)
{
    ...
    return 0 == memcmp(result_array, exp,
