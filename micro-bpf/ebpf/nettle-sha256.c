...
/* ==========================================
   ...
   Details:
      ...
      - added main
      - inlined functions
      - moved the nettle_sha256 into the benchmark_body to avoid relocations in the rodata.
   ========================================== */
    ...
    do {                                                                                                                  \
       ...
       is always at least one byte free */                                             \
        ...
        (ctx)->block[__md_i++] = 0x80;                                                                                    \
                                                                                                                          \
        if (__md_i > (sizeof((ctx)->block) - (size))) { /* No room for length in this block. Process it and             \
...
void _nettle_sha256_compress(uint32_t *state, const uint8_t *input,
                             const uint32_t *k)
{
...
unsigned char hash[32] = { 0x24, 0x8d, 0x6a, 0x61, 0xd2, 0x06, 0x38, 0xb8, 0xe5, 0xc0, 0x26, 0x93,
...
static int benchmark_body(unsigned int lsf);
...
static inline int benchmark_body(unsigned int lsf)
{
    struct nettle_hash nettle_sha256 = _NETTLE_HASH(sha256, SHA256);
