...
unsigned char msg[56] = "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq";
...
static inline void write_uint64(uint8_t *dst, uint64_t src)
{
    dst[0] = (src >> 56) & 0xff;
    dst[1] = (src >> 48) & 0xff;
    dst[2] = (src >> 40) & 0xff;
...
static inline void write_uint32(uint8_t *dst, uint32_t src)
{
    dst[0] = (src >> 24) & 0xff;
    dst[1] = (src >> 16) & 0xff;
    dst[2] = (src >> 8) & 0xff;
...
static inline uint32_t read_uint32(const uint8_t *src)
{
...
static inline void sha256_write_digest(struct sha256_ctx *ctx, size_t length, uint8_t *digest)
{
    uint64_t bit_count;
...
const struct nettle_hash nettle_sha256 = {
  ...
  .init         = (nettle_hash_init_func *) sha256_init,
  .update       = (nettle_hash_update_func *) sha256_update,
  .digest       = (nettle_hash_digest_func *) sha256_digest
};
